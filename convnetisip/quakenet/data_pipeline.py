...
class DataWriter(object):
    ...
    def write(self, sample_window, cluster_id):
        n_traces = len(sample_window)   #2 in ourcase
        n_samples = len(sample_window[0].data)   #150
        # Extract data
        data = np.zeros((n_traces, n_samples), dtype=np.float32)
        for i in range(n_traces):
            data[i, :] = sample_window[i].data[...]
        # Extract metadata
        start_time = np.int64(sample_window[0].stats.starttime.timestamp)     #NA/REC
        end_time = np.int64(sample_window[0].stats.endtime.timestamp)         #NA/REC
        # print('starttime {}, endtime {}'.format(UTCDateTime(start_time),
                                                # UTCDateTime(end_time)))

        example = tf.train.Example(features=tf.train.Features(feature={
            'window_size': self._int64_feature(n_samples),    #150
            'n_traces': self._int64_feature(n_traces),        #2
            'data': self._bytes_feature(data.tobytes()),      #normalized data
            'cluster_id': self._int64_feature(cluster_id),    #-1,0? or 0/1
