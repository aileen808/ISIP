...
class ConvNetISIP(tflib.model.BaseModel):
  ...
  def __init__(self, inputs, config, checkpoint_dir, is_training=False,
    ...
    self.config = config
    self.is_training = is_training
    super(ConvNetISIP, self).__init__(inputs, checkpoint_dir, is_training=is_training,
                                    reuse=reuse)
  ...
  def _setup_prediction(self):
    ...
    for i in range(depth):
        ...
        #current_layer = layers.batch_norm(current_layer,center=True,scale=True,scope='conv{}'.format(i+1))
        #Add batch_Normalization_layer(could be tested later)
...
#        'localization_accuracy': self.localization_accuracy
  ...
  def _setup_loss(self):
    with tf.name_scope('loss'):
      ...
      targets = self.inputs['cluster_id']
