...
class DataWriter(object):
    ...
    def write(self, sample_window, cluster_id):
        n_traces = len(sample_window) -1  #3-1 in ourcase
        n_samples = len(sample_window[0])   #150 
        # Extract data
        data = np.zeros((n_traces, n_samples), dtype=np.float32)
        for i in range(n_traces):
            data[i, :] = sample_window[i+1,:]
        # Extract metadata
        start_time = np.int64(sample_window[0,0])     #NA/REC
        end_time = np.int64(sample_window[0,-1])         #NA/REC
        ...
        example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'cluster_id': self._int64_feature(cluster_id),    # 0/1
