...
def conv1d(input_, output_dim, 
    ...
    with tf.variable_scope(name):
        #print(input_.get_shape()[-1])  ---2 in ourcase
...
def deconv1d(input_, output_shape,
    ...
    with tf.variable_scope(name):
        ...
        w = tf.get_variable('w', [k, output_shape[-1], input_.get_shape()[-1]],
                            initializer=tf.random_normal_initializer(stddev=stddev))
        
        try:
            deconv = tf.contrib.nn.conv1d_transpose(input_, w, output_shape=output_shape,
                                stride = d)

        # Support for verisons of TensorFlow before 0.7.0
        except AttributeError:
            print("ahhhh")
            deconv = tf.nn.conv1d_transpose(input_, w, output_shape=output_shape,
                                stride = d)
